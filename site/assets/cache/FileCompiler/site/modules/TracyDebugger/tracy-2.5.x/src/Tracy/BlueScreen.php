...
class BlueScreen
{
	...
	{
		$this->collapsePaths[] = preg_match('#(.+/vendor)/tracy/tracy/src/Tracy$#', strtr(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy', '\\', '/'), $m)
			? $m[1]
			: \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy';
	}
	...
	public function render($exception)
	{
		if (Helpers::isAjax() && session_status() === PHP_SESSION_ACTIVE) {
			ob_start(function () {});
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy' . '/assets/BlueScreen/content.phtml');
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy' . '/assets/BlueScreen/page.phtml');
		}
	}
	...
	public function renderToFile($exception, $file)
	{
		if ($handle = @fopen($file, 'x')) {
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy' . '/assets/BlueScreen/page.phtml', false);
			ob_end_flush();
			ob_end_clean();
			fclose($handle);
		}
	...
	private function renderTemplate($exception, $template, $toScreen = true)
	{
		...
		$css = array_map('file_get_contents', array_merge([
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.5.x/src/Tracy' . '/assets/BlueScreen/bluescreen.css',
